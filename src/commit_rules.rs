...
fn test_starting_emoji(input: &str) -> bool {
    CommitType::iter_variants().any(|commit_type| input.starts_with(commit_type.emoji()))
}
...
pub fn check_message_with_emoji(
    message: &str,
) -> impl Iterator<Item = CommitRuleValidationResult> + '_ {
    ...
    let message_without_emoji =
        CommitType::iter_variants().fold(message, |message, commit_type| {
            let emoji = format!("{} ", commit_type.emoji());
            if message.starts_with(&emoji) {
                &message[emoji.len()..]
            } else {
                message
            }
        });
    iter::once(emoji_validation_result).chain(check_message(message_without_emoji))
}
